# Vigil: Runtime Health Auditor

**Runtime Health Auditor for TestForge Test Scripts**
...
Vigil is the execution engine that takes test scripts generated by testforge (Test Generator) and runs them in an isolated sandbox with comprehensive health monitoring. Unlike standard test runners, Vigil acts as a **runtime auditor**, watching not just whether tests pass, but whether the application under test remains healthy during execution.

**Key Differentiator:**
- **Standard Runner**: Test passes ✓
- **Vigil**: Test passes ✓ BUT browser memory leaked 500MB ✗ → FAIL
...
Vigil assigns a health grade based on observed metrics during test execution:
...
Vigil uses a **sidecar monitoring approach** rather than running monitoring inside the executor container.
